...
function transportId(file, options, isjs){
  ...
  if (file.package && file.package.id) {
    ...
    id = util.slashPath(isjs ? util.hideExt(id) : id);
...
function transportDeps(file, options){
  ...
  file.contents = new Buffer(udeps(file.contents.toString(), function (id, flag){
    ...
    // if the id ends with `/'`, just return it without 'index.js'
    if (charCode === 47 /* "/" */) {
      id = id + 'index.js';
    } else
    // if the id not ends with `/' '.js' or not has '?', just return it with '.js'
    if (id.substring(last - 2) !== '.js' && id.indexOf('?') === -1 /* "?" */) {
      normalizeId = id + '.js';
    }
  ...
  file.package = util.extend(file.package, { dependencies: deps, localDepsMaps: localDepsMaps });
...
function transportCssDeps(file, options, css2js){
  ...
  file.contents = new Buffer(cssImports(file.contents.toString(), function (meta){
    ...
    // if the id ends with `/'`, just return it without 'index.css'
    if (charCode === 47 /* "/" */) {
      id = id + 'index.css';
    }
