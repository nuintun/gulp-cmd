...
function transportDeps(file, options){
  ...
  file.contents = new Buffer(udeps(file.contents.toString(), function (id, flag){
    ...
    // parse id form alias
    if (alias[normalizeId] && is.string(alias[normalizeId])) {
    ...
    // rename path
    path = rename.parse(path);
    path = (isRelative ? './' : '') + rename(path, renameOpts);
    ...
    if (flag === null) {
      ...
      // get absolute path
      path = join(file.base, util.resolve(path, file.package.id));
    ...
    // debug
    debug('transport %s dependencies ( %s )', flag || 'require', colors.dataBold(id));
