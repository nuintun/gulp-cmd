...
function transportId(file, options){
  ...
  var prefix = slashPath(template(idleading, pkg));

  console.log(pkg);
  console.log(prefix);

  var fileObj = rename.parse(file.path);
  var renameOpts = util.getRenameOpts(file, options.rename);
  var fullpath = rename(fileObj, renameOpts);
  ...
  //var id = slashPath(join(prefix, filepath));
