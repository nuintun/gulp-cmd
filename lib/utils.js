...
function initIgnore(options) {
  ...
  // Format ignore
  Array.isArray(options.ignore) && options.ignore.forEach((id) => {
    // Compute id
...
function transportDeps(vinyl, options) {
  ...
  vinyl.contents = new Buffer(cmd(vinyl.contents.toString(), (id, flag) => {
    if (gutil.isLocal(id)) {
      const src = id;
...
function transportCssDeps(vinyl, options) {
  ...
  const prefix = options.css.prefix;
