 ...
 * @returns {object}
 */
function parsePackage(file, wwwroot){
  var relative = file.relative;

  // file not in base dir, user wwwroot
  if (isRelative(relative)) {
    relative = path.relative(wwwroot, file.path);
    // file not in wwwroot, throw error
    if (isRelative(relative)) {
      throwError('%s is out of bound of wwwroot %s.', normalize(file.path), normalize(wwwroot));
    }
...
function resolve(relative, file, wwwroot){
  ...
  // debug
  debug('of base path: %s', colors.dataBold(base));
