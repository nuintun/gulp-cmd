...
function transportId(vinyl, options) {
  ...
  // debug
  debug('module id: %s', gutil.colors.magenta(id));
...
function transportDeps(vinyl, options) {
  ...
  vinyl.contents = new Buffer(cmd(vinyl.contents.toString(), function(id, flag) {
    ...
    if (!gutil.isLocal(id)) {
      ...
      // debug
      debug('module%s deps: %s', flag ? ' ' + flag : '', gutil.colors.magenta(id));
...
function transportCssDeps(vinyl, options) {
  ...
  vinyl.contents = new Buffer(css(vinyl.contents, function(id) {
    ...
    if (!gutil.isLocal(id)) {
      ...
      path = id = addExt(id);
      // dependencie real path
      path = resolve(path, vinyl, options.wwwroot, true);
...
function hideExt(path) {
  ...
  if (/\.css\.js$/.test(path)) return path;
