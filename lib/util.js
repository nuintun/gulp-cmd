...
function parsePackage(file){
  ...
  var filename = hideExt(basename(file.relative));
  var SPLITRE = /\/((?:\d+\.){2}\d+)(?:\/|$)/;
...
function filepath(id){
  ...
  if (charCode === 35 /* "#" */) return id.substring(0, last);
...
function resolvePath(relative, base){
  ...
  debug('transport relative id ( %s ) of basepath ( %s )', colors.dataBold(relative), colors.dataBold(base));

  relative = join(dirname(base), relative);

  if (isRelative(relative)) {
    throwError('%s is out of bound of %s', slashPath(colors.dataBold(relative)), colors.dataBold(base));
  }
