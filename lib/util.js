...
function resolve(relative, vinyl, wwwroot){
  ...
  if (isRelative(path.relative(wwwroot, relative))) {
    throwError('file: %s is out of bound of wwwroot: %s.', normalize(relative), normalize(wwwroot));
  }

  // debug
  debug('of base path: %s', colors.dataBold(relativeCwd(base)));
  debug('to: %s', colors.dataBold(relativeCwd(relative)));
...
function relativeCwd(filepath){
  return normalize(path.relative(cwd, filepath));
}
...
exports.relativeCwd = relativeCwd;
