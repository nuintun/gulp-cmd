...
function plugin(type, transport){
  return function (vinyl, options){
    // debug
    debug('load plugin: %s', colors.input(type));
    // debug
    debug('read file: %s', colors.data(util.pathFromCwd(vinyl.path)));
...
function initPlugins(options){
  ...
  for (var key in inlinePlugins.defined) {
    if (inlinePlugins.defined.hasOwnProperty(key)) {
      if (customPlugins.hasOwnProperty(key)) {
        customPlugin = customPlugins[key];

        if (is.fn(customPlugin)) {
          customPlugin = plugin(key.substring(1), customPlugin);
        } else {
          throwError('options.plugins[' + key + '] should be function.');
        }
      } else {
        customPlugin = inlinePlugins.defined[key];
      }

      plugins.defined[key] = customPlugin;
    }
  }
...
function initOptions(options){
  ...
  // init plugins
  defaults.plugins = initPlugins(options);
