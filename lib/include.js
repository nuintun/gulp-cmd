...
function include(options){
  ...
  return through.obj({ objectMode: true }, function (vinyl, encoding, next){
    ...
    if (!initialized) {
      ...
      // debug
      debug('cwd: %s', colors.dataBold(util.normalize(util.cwd)));
...
function includeDeps(vinyl, options, included){
  ...
  function traverse(path, base){
    ...
    if (vinyl !== null) {
      ...
      // debug
      debug('include: %s', colors.dataBold(util.relativeCwd(vinyl.path)));
...
function vinylFile(path, vinyl){
  ...
  debug('file: %s not exists', colors.warn(util.relativeCwd(path)));
